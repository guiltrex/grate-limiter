 ...
 * @constructor
 *
 * @param {Function} errHandler Optional custom error handler function.
 */
function RateLimiter(errHandler) {
  this._limits = {};
  this._limitsData = {};
  this._errHandler = errHandler || defaultErrorHanadler;
}
 ...
 *                            (defaults to false).
 */
RateLimiter.prototype.addLimit = function(path, method,clientId,
                                          requestCount,
                                          requestPeriod,
                                          returnErr) {
  method = method.toLowerCase() || 'all';
  var key = this._getKeyForLimit(path, method, clientId);
 ...
 * @param {String} method HTTP method name or 'all' for all.
 */
RateLimiter.prototype.removeLimit = function(path, method, clientId) {
  var key = this._getKeyForLimit(path, method, clientId);

  if (!this._limits.hasOwnProperty(key)) {
    throw new Error(sprintf('Limit for path %s, method %s and client %s does not exist',
                     path, method, clientId));
  }
...
/**
 * Reset access counter for the provided clientId.
 *
 * @param {RegExp} path Regular expression for the path.
 * @param {String} method HTTP method name or 'all' for all.
 * @param {String} clientId Client ID for which the counter will be reset.
 */
RateLimiter.prototype.resetClientIdAccessCounter = function(path, method,
                                                             clientId) {
  var key = this._getKeyForLimit(path, method, clientId);
  ...
  if (!this._limitsData[key].hasOwnProperty(clientId)) {
    throw new Error(sprintf('No recorded data for clientId %s exists.', clientId));
  }

  this._limitsData[key][clientId]['access_count'] = 0;
};
 ...
 * @param {String} method HTTP method name or 'all' for all.
 */
RateLimiter.prototype._getKeyForLimit = function(path, method, clientId) {
  var key = sprintf('%s.%s.%s', path.toString(), method.toLowerCase(), clientId.toLowerCase());
  return key;
};
...
RateLimiter.prototype.processRequest = function(req, res, callback) {
  ...
  var clientId, _ref, _ref1, _ref2;
  clientId = (_ref = (_ref1 = req.query) != null ? (_ref2 = _ref1.clientid) != null ? _ref2.toLowerCase() : void 0 : void 0) != null ? _ref : 'invalid';
  
  var now = Math.round(new Date() / 1000);
  var limitReached = false;
  var limit, limitData, ipLimitData, code, headers, errMsg;

  ckey = this._getKeyForLimit(path, method, clientId);
  if (!this._limits[ckey]) {
    clientId = 'invalid'
  }
  ...
  for (var key in this._limits) {
    if (this._limits.hasOwnProperty(key)) {
      ...
      if (!limitData.hasOwnProperty(clientId)) {
        limitData[clientId] = {
          'access_count': 0,
          'expire': null
        };
      ...
      ipLimitData = limitData[clientId];

      if (!path.match(limit['path_re']) || (limit['method'] !== 'all' &&
          limit['method'] !== method) || limit['clientId'] !== clientId) {
        continue;
      }
      else {
        ...
        if ((ipLimitData['access_count'] >= limit['request_count']) &&
          ...
          limitReached = true;
          this._errHandler(req, res, limit, ipLimitData);
        }
 ...
 */
function defaultErrorHanadler(req, res, limit, ipLimitData) {
  var headers, code, now, errMsg;
  ...
  res.end(JSON.stringify({'err' : errMsg}));
}

function expressMiddleware(rules, errHandler) {
  var i, rulesLen, rule;
  var limiter = new RateLimiter(errHandler);
